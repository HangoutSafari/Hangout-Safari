...
export class Animal extends THREE.Mesh {
  ...
  public constructor(
    ...
    loadModel(path).then((loadedModel) => {
      const geometry = (loadedModel.scene.children[0] as THREE.Mesh).clone()
        .geometry;
      const material = (loadedModel.scene.children[0] as THREE.Mesh).clone()
        .material;
  ...
  public lsd_update() {
    ...
    const verticalOffset = Math.min(
      Math.sin(this.animationScale) * jumpHeight - 2,
      jumpHeight
    );
    ...
    (this.material as THREE.MeshBasicMaterial).color.setRGB(
      colorVariation,
      1 - colorVariation,
      1
    ); // Adjust the color properties as needed
  }

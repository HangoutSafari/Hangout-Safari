...
{#if showPopup}
   <div class="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 flex items-center justify-center z-50">
       <div class="background p-10 rounded shadow-xl">
           <p class="text-lg">Are you sure you want to delete this?</p>
            <div class="flex justify-between mt-4 font-bold">
                <button class="bg-green-500 text-white py-2 px-5 rounded"> YES </button>
                <button class="bg-red-500 text-white py-2 px-5 rounded" on:click={() => showPopup = false}>NO</button>
            </div>
       </div>
   </div>
{/if}

...
export const load = async (serverLoadEvent) => {
  ...
  try {
    ...
    let res = await fetch(
      `http://apigateway:3010/users/dashboards/${await session.user.id}/`
    );
    const user = await res.json();

    res = await fetch(`http://apigateway:3010/users/dashboards/`);
    const users = await res.json();

    res = await fetch(`http://apigateway:3010/animals/auth/`, {
      credentials: "include",
    });

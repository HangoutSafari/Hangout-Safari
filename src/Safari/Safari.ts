...
export class Safari {
  ...
  constructor(
    ...
    safariModel: string,
    animalEventDispatcher: AnimalEventDispatcher
  ) {
  ...
  public processMouseMoveEvent(event) {
    ...
    this.mousePos.x =
      ((event.clientX - this.scene.renderer.domElement.offsetLeft) /
        this.scene.renderer.domElement.clientWidth) *
        2 -
      1;
    this.mousePos.y =
      ((event.clientY - this.scene.renderer.domElement.offsetTop) /
        this.scene.renderer.domElement.clientHeight) *
        -2 +
      1;
  }
